...
```text
snarkyctl-0.10.0.dev2-py3-none-any.whl
```
...
`0.10.0~dev2-1` so it sorts before a future `0.1.0-1`. The build helper checks this
...
For release `0.10.0.dev2`, for example, `pyproject.toml` must contain `0.10.0.dev2` and the
changelog must begin with a version such as `0.10.0~dev2-1`. A mismatch exits with status 2
...
```bash
sudo scripts/reinstall-deb.sh ../snarkyctl_0.10.0~dev2-1_amd64.deb
```
