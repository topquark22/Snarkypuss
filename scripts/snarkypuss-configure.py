...
def render_dnsmasq_systemd(config: GatewayConfig) -> str:
    wireguard_unit = f"wg-quick@{config.tunnel_interface}.service"
    return (
        "# Generated by scripts/snarkypuss-configure.py\n"
        "[Unit]\n"
        "Description=Snarkypuss private DNS resolver\n"
        f"Requires={wireguard_unit}\n"
        f"After={wireguard_unit}\n"
        "\n"
        "[Service]\n"
        "Type=simple\n"
...
def main() -> int:
    ...
    try:
        ...
        dnsmasq_path = rooted(root, "/etc/snarkypuss/dnsmasq.conf")
        dnsmasq_systemd_path = rooted(
            root, "/etc/systemd/system/snarkypuss-dns.service"
        )
        ...
        targets = (
            ...
            (dnsmasq_path, "Snarkypuss dnsmasq configuration", 0o644),
            (dnsmasq_systemd_path, "Snarkypuss DNS systemd unit", 0o644),
        ...
        dnsmasq_systemd_result = atomic_write(
            dnsmasq_systemd_path,
            render_dnsmasq_systemd(config),
            0o644,
            timestamp,
        )
        ...
        print(f"{dnsmasq_systemd_result.upper()}: {dnsmasq_systemd_path}")
