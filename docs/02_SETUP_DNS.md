...
- `bind-interfaces` — bind only to the interfaces explicitly configured for `dnsmasq`.
...
It makes `dnsmasq.service` depend on the WireGuard service so that the private interface is
available before `dnsmasq` tries to bind to `10.8.0.1`.
...
- the systemd drop-in refers to `wg-quick@wg0.service`.
...
If this command reports an error, stop here. Do not repeatedly restart the service until the
configuration error has been understood.

## 4. Confirm that the WireGuard address exists

`dnsmasq` cannot listen on `10.8.0.1` if the WireGuard interface is not present.
...
If `wg0` does not exist or does not have that address, the problem is in the WireGuard or
networking setup rather than DNS itself. Return to [01_SETUP_VPS.md](01_SETUP_VPS.md).
...
The detailed diagnosis belongs in the troubleshooting guide rather than this setup guide.
When `05_TROUBLESHOOTING.md` is added, DNS service failures will be covered there.
