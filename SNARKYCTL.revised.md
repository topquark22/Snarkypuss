...
## Milestone 9: Provider-Neutral Target Administration

Deliverables:

- Add, edit, and remove approved VPN targets through an authenticated dashboard.
- Keep the ordinary connection endpoint alias-only.
- Let each installed adapter describe its target fields through declarative metadata so
  the dashboard does not contain provider-specific JavaScript.
- Validate target data at both the web boundary and privileged daemon boundary.
- Write the root-owned target catalogue atomically with correct ownership and mode.
- Preserve a rollback copy and reject symlinks, duplicate aliases, option-like values,
  extra fields, and unsafe lengths.
- Serialize catalogue mutations against VPN control operations and refresh the dashboard
  catalogue after a successful change.
- Record a controlled audit event without logging credentials or private provider data.

The administrative API necessarily receives provider-specific target fields when creating
a record, but those fields never pass directly to a command. The installed adapter first
validates and normalizes them into a root-owned catalogue entry. Subsequent connect
requests continue to submit only its public alias.

## Milestone 10: Additional Trusted Providers

Deliverables:

- Define the complete adapter contract for capabilities, status, settings, target
  validation, connect, disconnect, timeouts, and controlled errors.
- Add an adapter for administrator-supplied OpenVPN-compatible configurations.
- Add a Mullvad adapter using a supported Mullvad client or configuration mechanism.
- Provide adapter-specific preflight checks and representative parser fixtures.
- Normalize every adapter into the same browser-facing states and gateway modes.
- Verify that each provider owns its routing and leak-protection behavior without adding
  provider-specific routing logic to SnarkyCtl.
- Allow selection only among adapters installed in the trusted package registry.

Provider installation is not a browser upload feature. New executable code, command
paths, or plugins must arrive through a reviewed SnarkyCtl package or software upgrade.
Configuration may select an installed adapter but cannot import a module or define a
command.
...
- Provider-neutral target catalogue editor.
- Selection among installed, trusted provider adapters.
